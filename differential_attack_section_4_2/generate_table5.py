...
LIMIT = (3 * 3) + (2 * 8)  # 25
HM = [1, 12, 12, 10, 6, 12, 12, 10, 10, 8, 8, 12, 10, 8, 8, 4]
...
for i0 in range(12):
    ...
    for item0, item1, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11 in product(*current_loops):
        ...
        # 注意：保持你原代码中的位拼接顺序
        IN = (
                (A11 << 124) | (A10 << 108) | (A9 << 96) | (A8 << 88) |
                (A7 << 76) | (A6 << 64) | (A5 << 48) | (A4 << 44) |
                (A3 << 32) | (A2 << 20) | (A1 << 8) | (A0 << 0)
        )
        ...
        if COUNT > log2_res:
            ...
            print(f"B_weights={[item[1] for item in (item0, item1, item2, item3, item4, item5, item6, item7, item8, item9, item10, item11)]}")
