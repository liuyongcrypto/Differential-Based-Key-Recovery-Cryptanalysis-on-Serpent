...
# 基础全集
AAA_sets = [
    [0x4, 0x2, 0x6, 0xa, 0xc, 0xd, 0xf],
    [0x7, 0x1, 0x3, 0x5, 0x6, 0x8, 0x9],
    [0xe, 0x5, 0x6, 0x9, 0xb, 0xc, 0xd],
    [0x4, 0x1, 0x6, 0x8, 0x9, 0xa, 0xc],
    [0x2, 0x4, 0x5, 0x9, 0xb, 0xc, 0xf],
    [0x1, 0x4, 0x5, 0x6, 0x8, 0xa, 0xb, 0xf],
    [0x8, 0xf, 0x5, 0x7, 0xc, 0xe],
    [0x2, 0x4, 0x5, 0x9, 0xb, 0xc, 0xf],
    [0xa, 0x2, 0x3, 0x5, 0x7, 0x8, 0xb],
    [0x6, 0xc, 0x3, 0x5, 0x9, 0xf]
]
...
# 子集
AA_sets = [
    [0x4], [0x7], [0xe], [0x4], [0x2], [0x1, 0x4, 0x5, 0x6, 0x8, 0xa, 0xb, 0xf],
    [0x8, 0xF], [0x2], [0xa], [0x6, 0xc]
]
...
LIMIT = (3 * 3) + (2 * 8)  # 25
HM = [1, 6, 10 , 10, 10, 10, 10, 10,10, 10, 10, 10, 12, 4, 10, 10]
        ...
        for i2 in range(i1 + 1, 10):
            # for i3 in range(i2 + 1, 10):
                # for i4 in range(i3 + 1, 10):
                    ...
                    for item0, item1, item2, item3, item4, item5, item6, item7, item8, item9 in product(*current_loops):
                        ...
                        A0, A1, A2, A3, A4, A5, A6, A7, A8, A9 = (
                            ...
                            item6[0], item7[0], item8[0], item9[0]
                        )

                        # 注意：保持你原代码中的位拼接顺序
                        IN = (
                                (A9 << 116) | (A8 << 100) |
                                (A7 << 96) | (A6 << 88) | (A5 << 84) | (A4 << 76) |
                                (A3 << 72) | (A2 << 52) | (A1 << 36) | (A0 << 24)
                        )
                        ...
                        if COUNT > log2_res:
                            ...
                            print(f"B_weights={[item[1] for item in (item0, item1, item2, item3, item4, item5, item6, item7, item8, item9)]}")
